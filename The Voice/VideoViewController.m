...
- (void)saveLikeData {
    PFObject* newPhotoObject = [PFObject objectWithClassName:@"Like"];
    [newPhotoObject setObject:self.userID forKey:@"userId"];
    [newPhotoObject setObject:@"t" forKey:@"like"];
    [newPhotoObject setObject:self.videoKey forKey:@"videoKey"];
...
-(void)buttonLikeAction{
    if ([self.userID isEqualToString:@""]) {
        UIAlertView *message = [[UIAlertView alloc]initWithTitle:@"Information" message:@"Please Register with Facebook Account." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [message show];
        return;
    }
    ...
    [query whereKey:@"videoKey" equalTo:self.videoKey];
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error) {
            // The find succeeded.
            ...
            if (objects.count >= 1) {
                ...
                if (like == false) {
                    ...
                    [query getObjectInBackgroundWithId:self.user block:^(PFObject *imageLike, NSError *error) {
                        imageLike[@"like"] = @"f";
                        [imageLike saveInBackground];
                    }];
                    like = true;
                    
                    PFQuery *updateLikeCountQuery = [PFQuery queryWithClassName:@"VideoTables"];
                    [updateLikeCountQuery whereKey:@"videoKey" equalTo:self.videoKey];
                    [updateLikeCountQuery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                        if (!error) {
                            if (objects.count >= 1) {
                                for (PFObject *object in objects) {
                                ...
                                [updateLikeCountQuery getObjectInBackgroundWithId:objID block:^(PFObject *getLike, NSError *error) {
                                    int score = [[getLike objectForKey:@"countLike"] intValue];
                                    
                                    if (score <= 0) {
                                        [updateLikeCountQuery getObjectInBackgroundWithId:objID block:^(PFObject *imageLike, NSError *error) {
                                            imageLike[@"countLike"] = @"0";
                                            [imageLike saveInBackground];
                                        }];
                                    }
                                        ...
                                        score = score - 1;
                                        countLabel.text = [NSString stringWithFormat:@"%d",score];
                                        [updateLikeCountQuery getObjectInBackgroundWithId:objID block:^(PFObject *imageLike, NSError *error) {
                                            imageLike[@"countLike"] = [NSString stringWithFormat:@"%d",score];
                                            [imageLike saveInBackground];
                                        }];
                                    }
